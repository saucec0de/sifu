        ...
        for report in test_case_report_list:
            ...
            # output_match == None means the user might have tried to print to outfile
                ...
                if report['output_match'] == "all_failed_test_cases_had_single_element_array":
                    util.addFinding("A test case failed!", 0, "MAX_GREATER_BY_K_IN_ARRAY__SINGLE_ELEMENT", "TEST_100003")
...
def source_code_analysis(config, overall_report):
    ...
    with open(filename, 'r') as f:
        for line in f:
            if re.search(r"\barr\[i\] *\- *max\b", line):
                overall_report['source_code__arr_i_minux_max'] = True
            if re.search(r"\barr\[i\] *\- *second_max\b", line):
                overall_report['source_code__arr_i_minux_second_max'] = True
            if re.search(r"\bsecond_max *\+ *k\b", line):
                overall_report['source_code__second_max_plus_k'] = True

            if re.search(r"\bif\b *\(.*\) *;", line):
                overall_report['source_code__if_semicolon'] = True
