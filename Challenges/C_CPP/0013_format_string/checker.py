    ...
    if 'runtime_analysis_done' in overall_report:
        ...
        for report in test_case_report_list:
            ...
            # output_match == None means the user might have tried to print to outfile
                ...
                util.addFinding("A test case failed! Make sure you are not trying to print something extra.",
                                0,"","TEST_100006")
            elif not all(report['output_match']): # not all test cases passed
                util.addFinding("A test case failed!", 0, "", "TEST_100007")
                if 'source_code__improper_string_equality_check' in overall_report:
                    util.addFinding("A test case failed!", 0, 'INCREMENTAL_3_FORMAT_STRING__STRING_EQUALITY_', "TEST_100005")
            else:
                success_count += 1
...
def source_code_analysis(config, overall_report):
    ...
    with open(filename, 'r') as f:
        for line in f:
            if re.search(r"\bif\b.*\buser_answer\b *== *\bexpected_answer\b", line):
                overall_report['source_code__improper_string_equality_check'] = True
